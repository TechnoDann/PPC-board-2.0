# Usage: rake db:post_archive FILE=filename LINK_MAP_FILE=filename
namespace :db do
  ...
  task :post_archive => :environment do
    ...
    def post_archive(archive, parent, user, link_map_file, root_date)
      parent_id = nil
      archive.each do |entity|
        if entity.is_a?(Hash)
          safe_hash = entity.remove(:timestamp, :source).merge!({:parent_id => parent})
          ...
          if root_date == nil
            root_date = entity[:timestamp]
          end
          if entity.has_key? :source
            link_map_file << [entity[:source], parent_id, entity[:timestamp], root_date]
          end
        elsif entity.is_a?(Array)
          unless entity == []
            # Subthread of the post we last posted
            post_archive(entity, parent_id, user, link_map_file, root_date)
          end
        else
          puts("Can't happen")
        end
    ...
    def post_threads
      ...
      link_map_file = CSV.open(ENV['LINK_MAP_FILE'] || "link_map.csv", "w")
      link_map_file << ["source", "id", "date", "root_date"]
      board.each do |thread|
        post_archive thread, nil, user, link_map_file, nil
        true
      end
      link_map_file.close()
      true
